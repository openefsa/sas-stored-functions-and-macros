...
This will print inside the log a line for each facet with the facet information:
    ...
    data _null_;
      %facetIter("F01.BBB$F02.CCC", fac, head, code)
...
Example of false assertion:
    ...
    %assertExists("BBB");  /* This will print an error in the log */
----------
## **assertEqualsValue**
...
Interface for applying a general data enrichment and validation step. It enriches and validates the *inputTable* according to the required *action.* Each action has its own set of parameters which must be specified. Note that this macro can be used without specifying the whole set of parameters, since each action has its own set. For this reason, it is needed to specify the name of the declared parameter (see Examples for further details).
**Parameters:**
  ...
  foodex2Column - Name of a column of the *inputTable* containing FoodEx2 codes. In particular it is mandatory for the following actions:
    - “FOODEX2_VALIDATION”, the *foodex2Column* indicates which is the FoodEx2 column to validate
    - “FOODEX2_TO_MATRIX”, the *foodex2Column* indicates the FoodEx2 column which will be converted in matrix code
    
  foodex2Hierarchy - Code of a FoodEx2 hierarchy. In particular, it can be used only for action = “FOODEX2_VALIDATION”. It specifies in which hierarchy the parent-child relationships of the FoodEx2 terms should be evaluated. This allows performing additional checks to validate FoodEx2 codes. Note that this parameter is ***optional*** and, if omitted, the hierarchy-dependent checks will not be performed (as the FOODEX.19).
  
  matrixColumn - Name of a column of the *inputTable* which contains or will contain a matrix code. In particular it is mandatory for the following actions:
    - “FOODEX2_TO_MATRIX”, the *matrixColumn* specifies the name of the column which will contain the mapped matrix codes
...
It validates the “FX2” FoodEx2 column of the MY_DATA dataset. The validation results are returned in MY_ERRORS.
    ...
    run;
    %DEAV_ENRICH_AND_VALIDATE(inputTable=MY_DATA, outputTable=MY_ERRORS, action="FOODEX2_VALIDATION", foodex2Column=FX2);
...
It validates the “FX2” FoodEx2 column of the MY_DATA dataset using the reporting hierarchy as parent-child relationships evaluator. The validation results are returned in MY_ERRORS.
    ...
    data MY_DATA;
      ...
      FX2 = "A029F#F20.A07QQ$F21.A07RV"; output;
    run;
    %DEAV_ENRICH_AND_VALIDATE(inputTable=MY_DATA, outputTable=MY_ERRORS, action="FOODEX2_VALIDATION", foodex2Column=FX2, foodex2Hierarchy="REPORT");
...
It maps the “FX2” FoodEx2 column of the MY_DATA dataset into matrix codes which will be inserted in a new column named “MATRIX”.
    ...
    data MY_DATA;
      ...
      FX2 = "A029F#F20.A07QQ$F21.A07RV"; output;
    run;
    %DEAV_ENRICH_AND_VALIDATE(inputTable=MY_DATA, outputTable=MY_ERRORS, action="FOODEX2_TO_MATRIX", foodex2Column=FX2, matrixColumn=MATRIX);
----------
## **DEAV_FOODEX2_VALIDATION**


    %macro DEAV_FOODEX2_VALIDATION(input, output, fx2v_foodex2Column, fx2v_foodex2Hierarchy);

Validates a set of FoodEx2 codes and returns the errors/warnings. This is the same as calling the DEAV_ENRICH_AND_VALIDATE macro with action = “FOODEX2_VALIDATION” (which is the preferred method). 
**Parameters:**

  input - Input table containing the data to validate
  output - Output table which will contain the detected errors. This table will be created by the algorithm.
  fx2v_foodex2Column - The name of the column of *input* containing the FoodEx2 code to validate
  fx2v_foodex2Hierarchy - Optional parameter, it specifies which hierarchy should be used for performing additional validation steps, as the FOODEX.19 business rule. If omitted, the additional checks are skipped.
...
**Examples:**
    ...
    run;
    %DEAV_FOODEX2_VALIDATION(MY_DATA, ERR_TABLE, FOODEX); /* without hierarchy */
    %DEAV_FOODEX2_VALIDATION(MY_DATA, ERR_TABLE, FOODEX, "REPORT"); /* with reporting hierarchy */
----------
...
----------
## **DEAV_INT_TO_BR_ERRORS**


    %macro DEAV_INT_TO_BR_ERRORS(DEAV_errorTable, ruid_col, BR_errorTable)

Converts an error table generated from a data enrichment and validation step into a table which has a format suitable for the business rules.
**Parameters:**

  DEAV_errorTable - Name of the DEAV error table to convert
  ruid_col - Name of the column which univocally identifies a record of the *DEAV_errorTable*. This could be for example the record unique identifier of the validated row (e.g. resId)
  BR_errorTable - Name of the BR error table to create
----------
# **Functions**
